...
# В нашей упрощенной реализации разобьем дальнейшую иерархию на Лодку (`Boat`)
# и Колесное Транспортное Средство (`Wheeled Vehicle`).
# `Wheeled Vehicle` далее можно разбить на Машину (`Car`) и
# Велосипед (`Bicycle`), а Машину разобьем на Купе (`Coupe`) и Седан (`Sedan`).
#
